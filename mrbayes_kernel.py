...
class MrBayesKernel(ProcessMetaKernel):
    implementation = 'MrBayes Kernel'
    ...
    def banner(self):
        ...
        #    executable = os.environ.get('MRBAYES_EXECUTABLE', 'mb')
        #    banner = subprocess.check_output([executable, '--version'])
        #    self._banner = banner.decode('utf-8')
        return self._banner

    def makeWrapper(self):
        """Start a MrBayes process and return a :class:`REPLWrapper` object.
        """
        #if os.name == 'nt':
        #    orig_prompt = u(chr(3))
    ...
    #        self.handle_plot_settings()
            super(MrBayesKernel, self).do_execute_direct(self._setup)
    #        if os.name != 'nt':
    ...
    #                self.Error(msg)
        ...
        resp = super(MrBayesKernel, self).do_execute_direct(code)
...
if __name__ == '__main__':
    ...
    IPKernelApp.launch_instance(kernel_class=MrBayesKernel)
